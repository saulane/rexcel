...
impl Editor {
    ...
    fn paste(&mut self, p: &Position) {
        if self.clipboard.is_some() {
            self.document
                .insert_cell(p, self.clipboard.as_ref().unwrap());
    ...
    fn move_cursor(&mut self, key: KeyCode) {
        ...
        match key {
            ...
            KeyCode::Down => {
                if self.cell_position.y.saturating_sub(self.offset.y)
    ...
    fn goto(&mut self) {
        let previous_pos = self.cell_position.clone();
        let _query = self
            .prompt(
                "Go To (row,column): ",
                |editor: &mut Editor, key: KeyCode, query: &String| {
                    let mut split = query.split(',');
                    let x = split
                        .next()
                        .unwrap_or(previous_pos.x.to_string().as_str())
                        .parse::<usize>()
                        .unwrap_or(previous_pos.x);
                    let y = split
                        .next()
                        .unwrap_or(previous_pos.y.to_string().as_str())
                        .parse::<usize>()
                        .unwrap_or(previous_pos.y);
    ...
    fn teleport(&mut self, p: &Position) {
        self.cell_position = *p;
        let mut new_offset: Position = Position::default();
        new_offset.y = p.y.saturating_sub(self.terminal.size().height);
    ...
    fn draw_edit_line(&mut self) -> Result<(), std::io::Error> {
        ...
        if self.document.cell_exist(curr_pos) {
            write!(
                stdout(),
                "{}",
                self.document.get_cell(curr_pos).unwrap().val
            )
        } else {
            write!(stdout(), "")
        }
    ...
    fn draw_status_message(&mut self) -> Result<(), std::io::Error> {
        ...
        let helper_message =
            "CTRL + Q: Quit | CTRL + S: Save | CTRL + ALT + S: Save as".to_string();
        ...
        if size.width > 2 * helper_message_len {
            Terminal::goto(&Position {
                x: size.width.saturating_sub(helper_message_len),
                y: 0,
            });
            Terminal::set_bg_color(Color::White);
            Terminal::set_fg_color(Color::Black);
    ...
    fn draw_header(&mut self) -> Result<(), std::io::Error> {
        ...
        let cols: usize = max(
            self.document.col_count(),
            self.cell_position.x.saturating_add(1),
        );

        let headers: Vec<String> = if self.header {
            let col_titles = self.document.rows[0]
                .cells
                .iter()
                .map(|c| c.render(9))
                .collect::<Vec<String>>();
            ...
            let margin = lens
                .iter()
                .map(|l| 9_usize.saturating_sub(*l) / 2)
                .collect::<Vec<usize>>();
            let headers_str = col_titles
                .iter()
                .enumerate()
                .map(|i| {
                    format!(
                        ...
                        &" ".repeat(margin[i.0]),
                        i.1,
                        &" ".repeat(9_usize.saturating_sub(lens[i.0].saturating_add(margin[i.0])))
                    )
                })
                .collect::<Vec<String>>();
            headers_str
